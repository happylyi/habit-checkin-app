...
export default function RegisterPage() {
  ...
  async function onSubmit(e: FormEvent) {
    ...
    const emailConfirmationSent =
      typeof data === "object" && data !== null && "user" in data && (data as { user?: unknown }).user === null;
  ...
  return (
    <main className="min-h-[100dvh] bg-zinc-50">
      <div className="mx-auto max-w-md p-4">
        <h1 className="py-10 text-center text-2xl font-semibold">注册</h1>
        ...
        <form onSubmit={onSubmit} className="flex flex-col gap-3 rounded-2xl bg-white p-4 shadow-sm">
          ...
          {successText ? <div className="rounded-xl bg-emerald-50 p-3 text-sm text-emerald-700">{successText}</div> : null}
          ...
          <div className="py-2 text-center text-sm text-zinc-600">
            已有账号？{" "}
            <a className="font-medium text-zinc-900" href="/auth/login">
              去登录
            </a>
          </div>
        </form>
