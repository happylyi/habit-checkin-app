...
export default function TodayCheckinPage() {
  ...
  const [habitCheckinsByHabitId, setHabitCheckinsByHabitId] = useState<Record<string, HabitCheckinRow>>(
    {}
  );

  const [tasks, setTasks] = useState<TaskRow[]>([]);
  const [taskCheckinsByTaskId, setTaskCheckinsByTaskId] = useState<Record<string, TaskCheckinRow>>({});
  const [targetsByTaskId, setTargetsByTaskId] = useState<Record<string, TaskTargetRow>>({});
  ...
  async function refresh() {
    ...
    try {
      ...
      const [habitsRes, habitCheckinsRes, tasksRes, taskCheckinsRes, targetsRes] = await Promise.all([
        supabase
          .from("habits")
        ...
        supabase.from("task_checkins").select("task_id,done_count,note").eq("date", today),
      ...
      const habitCheckinsRows = (habitCheckinsRes.data ?? []) as HabitCheckinRow[];
      ...
      for (const h of habitsRows) nextHabitDraft[h.id] = habitMap[h.id]?.note ?? "";
  ...
  async function createTask() {
    ...
    if (taskId) {
      const { error: targetErr } = await supabase.from("task_daily_targets").upsert({
        user_id: userId,
        task_id: taskId,
  ...
  return (
    <section className="pb-24">
      <div className="rounded-2xl bg-white p-4 shadow-sm">
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-zinc-600">{todayKey}（本地时区 Asia/Shanghai）</p>
          </div>
          <div className="flex gap-2">
            ...
            >
              习惯
              <span className="ml-1 text-xs text-white/90">{mode === "habit" ? `(${habitCount})` : ""}</span>
            </button>
            <button
              type="button"
            ...
            >
              任务
              <span className="ml-1 text-xs text-white/90">{mode === "task" ? `(${taskCount})` : ""}</span>
            </button>
          </div>
        </div>
        ...
        {mode === "habit" ? (
          <>
            <div className="flex items-center justify-between">
              <h3 className="text-sm font-semibold">习惯打卡</h3>
            </div>
            ...
            <div className="mt-4 flex flex-col gap-3">
              {habits.length === 0 ? (
                <div className="text-sm text-zinc-600">暂无习惯。先添加一个吧。</div>
              ) : null}
              ...
              {habits.map((h) => {
                ...
                return (
                  <div key={h.id} className="rounded-2xl border border-zinc-200 p-3">
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        ...
                        <div className="mt-1 text-xs text-zinc-500">{checked ? "已完成" : "未完成"}</div>
                      </div>
                      <div className="flex gap-2">
                        {checked ? (
                          <button
                    ...
                    <div className="mt-3">
                      <label className="text-xs text-zinc-600">备注（可选）</label>
                      <textarea
                        value={noteDraft}
                        onChange={(e) => setHabitNoteDraft((prev) => ({ ...prev, [h.id]: e.target.value }))}
          ...
          <>
            <div className="flex items-center justify-between">
              <h3 className="text-sm font-semibold">任务打卡</h3>
            </div>
            ...
            <div className="mt-4 flex flex-col gap-3">
              {tasks.length === 0 ? <div className="text-sm text-zinc-600">暂无任务。先添加一个吧。</div> : null}
                ...
                return (
                  <div key={t.id} className="rounded-2xl border border-zinc-200 p-3">
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        ...
                        <div className="mt-1 text-xs text-zinc-500">
                          目标：至少 {target} 次 / 达成：{reached ? "是" : checked ? "否" : "未打卡"}
                        </div>
                      </div>
                    ...
                    <div className="mt-3 grid grid-cols-2 gap-2">
                      <label className="text-xs text-zinc-600">
                        ...
                        <input
                          ...
                          step={1}
                          value={doneDraft}
                          onChange={(e) => setTaskDoneDraft((prev) => ({ ...prev, [t.id]: Number(e.target.value) }))}
                          className="mt-1 h-10 w-full rounded-xl border border-zinc-200 px-2 text-sm outline-none focus:border-zinc-300"
                        />
                      </label>
                      <div className="flex items-end justify-end">
                        <div className="text-xs text-zinc-500">{checked ? "已存在今日记录" : "将创建今日记录"}</div>
                      </div>
                    </div>
                    ...
                    <div className="mt-3">
                      ...
                      <textarea
                        value={noteDraft}
                        onChange={(e) => setTaskNoteDraft((prev) => ({ ...prev, [t.id]: e.target.value }))}
                        placeholder="补充说明..."
      ...
      {loading ? <div className="mt-4 text-center text-xs text-zinc-500">加载中...</div> : null}
    </section>
  );
}
