...
export default function HistoryPage() {
  ...
  async function refreshUserAndLists() {
    ...
    try {
      ...
      setHabitNameById(
        Object.fromEntries(habitsRows.map((h) => [h.id, h.name])),
      );
      setTaskNameById(Object.fromEntries(tasksRows.map((t) => [t.id, t.name])));

      if (habitsRows.length > 0 && selectedHabitId === "all")
        setSelectedHabitId(habitsRows[0].id);
      if (tasksRows.length > 0 && selectedTaskId === "all")
  ...
  const todayWithinFilter = useMemo(
    () => todayKey >= filterFromKey && todayKey <= filterToKey,
    [todayKey, filterFromKey, filterToKey],
  );
  ...
  const canSubmit =
    Boolean(userId) &&
    (mode === "habit" ? selectedHabitId !== "all" : selectedTaskId !== "all");
    ...
    try {
      ...
      if (doneCount <= 0) {
        ...
        const { error } = await supabase
          ...
          .update({
            done_count: doneCount,
            note: editingTaskNoteDraft.trim() || null,
          })
          .eq("id", editingTaskId);
        if (error) throw error;
      }
  ...
  return (
    <section className="pb-24" data-oid="8hfv.3s">
      <div className="rounded-2xl bg-white p-4 shadow-sm" data-oid="i:mz0j4">
        <div
          className="flex items-start justify-between gap-4"
          data-oid="99yu.0d"
        >
          <div data-oid="e4loptm">
            <h2 className="text-lg font-semibold" data-oid="pqam0n-">
              打卡历史（流水增删改查）
            </h2>
            <p className="mt-1 text-sm text-zinc-600" data-oid="uwhuf0-">
              修改完成后，统计/日历会在下一次进入时同步更新。
            </p>
          </div>
          <span
      ...
      <div
        ...
        data-oid="xbsxsuv"
      >
        <div className="grid grid-cols-2 gap-2" data-oid="o11:6ju">
          {mode === "habit" ? (
            <label className="text-sm text-zinc-600" data-oid="td_:ytn">
              选择习惯
              <select
                ...
                data-oid="n6rcaou"
              >
                <option value="all" data-oid="5-m3wow">
                  全部
                </option>
                {habits.map((h) => (
                  <option key={h.id} value={h.id} data-oid="gv45l0f">
                    {h.name}
                  </option>
                ))}
              </select>
            ...
            <label className="text-sm text-zinc-600" data-oid="nwm.-z6">
              选择任务
              <select
                ...
                data-oid="ssr6frx"
              >
                <option value="all" data-oid="_vf99cw">
                  全部
                </option>
                {tasks.map((t) => (
                  <option key={t.id} value={t.id} data-oid="lee75.m">
                    {t.name}
                  </option>
                ))}
              </select>
        ...
        <div className="mt-3 grid grid-cols-2 gap-2" data-oid="9y6c_9m">
          <label className="text-sm text-zinc-600" data-oid="p8ys15v">
            开始日期
            <input
              type="date"
              value={filterFromKey}
          ...
          <label className="text-sm text-zinc-600" data-oid="e6rtiyh">
            结束日期
            <input
              type="date"
              value={filterToKey}
        ...
        <div
          ...
          data-oid="49i7hqq"
        >
          <div className="text-sm font-semibold" data-oid="re64gu4">
            新增/更新（按日期覆盖）
          </div>
          <div className="mt-2 grid grid-cols-2 gap-2" data-oid="vpzpv56">
            <label className="text-sm text-zinc-600" data-oid="kdwavy2">
              日期
              <input
                type="date"
                value={formDateKey}
            ...
            {mode === "habit" ? (
              <div className="flex flex-col" data-oid="a_rfjji">
                <span className="text-sm text-zinc-600" data-oid="djq126:">
                  备注（可选）
                </span>
                <input
              ...
              <div className="flex flex-col" data-oid="l5qnk.:">
                <span className="text-sm text-zinc-600" data-oid="h-5um6.">
                  完成次数（0=撤销）
                </span>
                <input
                  type="number"
          ...
          {mode === "task" ? (
            <div className="mt-2" data-oid="svoyxfa">
              <label className="text-sm text-zinc-600" data-oid="bhe9w63">
                备注（可选）
                <input
                  value={taskNoteDraft}
          ...
          <button
            ...
            onClick={() =>
              mode === "habit" ? upsertHabitCheckin() : upsertTaskCheckin()
            }
          ...
          {!canSubmit ? (
            <div className="mt-2 text-xs text-zinc-600" data-oid="haw10z6">
              {mode === "habit"
                ? "请先选择具体习惯（不是全部）"
                : "请先选择具体任务（不是全部）"}
            </div>
      ...
      >
        <div
          ...
          data-oid="-mn5.-5"
        >
          <div className="text-sm font-semibold" data-oid="0bisvl1">
            {mode === "habit" ? "习惯历史列表" : "任务历史列表"}
          </div>
          <div className="text-xs text-zinc-500" data-oid=":vd6mx1">
            第 {page + 1} 页
          </div>
        </div>

        {mode === "habit" ? (
          <div className="mt-3 flex flex-col gap-2" data-oid="ca-coiq">
            {habitRecords.length === 0 ? (
              <div className="text-sm text-zinc-600" data-oid="-:-gumv">
                暂无数据
              </div>
            ) : null}
              ...
              return (
                ...
                >
                  <div
                    ...
                    data-oid="2h0ez9b"
                  >
                    <div data-oid="1dbn:hs">
                      <div className="text-sm font-semibold" data-oid="0wrqn2y">
                        {name}{" "}
                        <span
                      ...
                      >
                        {isEditing ? (
                          <input
                            ...
                            onChange={(e) =>
                              setEditingHabitNoteDraft(e.target.value)
                            }
                    ...
                    <div className="flex gap-2" data-oid="6s2oed8">
                      {isEditing ? (
                        <>
                          <button
                            type="button"
                            ...
                            disabled={loading}
                            className="h-9 rounded-xl bg-zinc-900 px-3 text-xs text-white disabled:opacity-60"
                            data-oid="pc8ar.0"
                          >
                            保存
                          </button>
          ...
          <div className="mt-3 flex flex-col gap-2" data-oid="ffkbty:">
            {taskRecords.length === 0 ? (
              <div className="text-sm text-zinc-600" data-oid="bhav4-0">
                暂无数据
              </div>
            ) : null}
              ...
              return (
                ...
                >
                  <div
                    ...
                    data-oid="kkyk1hs"
                  >
                    <div data-oid="8wgoymg">
                      <div className="text-sm font-semibold" data-oid="gv7_1cx">
                        {name}{" "}
                        <span
                          className="ml-2 text-xs text-zinc-500"
                          data-oid="7zjpxjl"
                      ...
                      <div
                        className="mt-1 flex items-center gap-2 text-sm text-zinc-800"
                        data-oid="kgun_gs"
                      >
                        {isEditing ? (
                          <>
                            <input
                              ...
                              onChange={(e) =>
                                setEditingTaskDoneDraft(Number(e.target.value))
                              }
                      ...
                      >
                        {isEditing ? (
                          <input
                            ...
                            onChange={(e) =>
                              setEditingTaskNoteDraft(e.target.value)
                            }
                            className="w-full rounded-xl border border-zinc-200 bg-white px-3 py-2 text-sm outline-none"
                            data-oid=".q:2z8v"
                          />
                        ) : (
                          <span data-oid="k.c1adf">{row.note ?? "—"}</span>
                        )}
                      </div>
                    </div>

                    <div className="flex gap-2" data-oid=":pva1a3">
                      {isEditing ? (
                        <>
                          <button
                            type="button"
